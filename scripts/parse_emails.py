...
def manual_weeks(dest):
    """Load hand-entered weeks that sit alongside the parsed ones."""
    path = os.path.join(os.path.dirname(os.path.abspath(dest)), "manual-weeks.json")
    ...
    for w in raw.get("weeks", []):
        week = dict(w)
        # Provisional unless the entry explicitly says it was transcribed from a
        # real email — an omitted flag should never pass guesswork off as reported.
        week["placeholder"] = bool(w.get("placeholder", True))
        week.setdefault("source_note", None)
        ...
        if totals.get("engagement_rate") is None and totals["impressions"] and totals["engagements"]:
            totals["engagement_rate"] = round(totals["engagements"] / totals["impressions"] * 100, 1)
        week["totals"] = totals
        week["checks"], week["platform_rows_suspect"] = reconcile(week)
        weeks.append(week)
    return weeks
...
def main(src, dest):
    ...
    for path in glob.glob(os.path.join(src, "*.eml")):
        ...
        weeks[-1]["placeholder"] = False
    reported = {w["label"] for w in weeks}
    extra = [w for w in manual_weeks(dest) if w["label"] not in reported]
    weeks.extend(extra)
    weeks.sort(key=lambda w: w["week_start"])
    payload = {
        "brand": "Crooked Ideas",
        "source": "Crooked Media Weekly Social Analytics emails",
        "generated_from": len(weeks) - len(extra),
        "manual_weeks": len(extra),
        "placeholder_weeks": sum(1 for w in extra if w["placeholder"]),
        "weeks": weeks,
    }
    ...
    if os.path.isdir(os.path.dirname(js)):
        ...
        print("wrote %s" % js)
    print("wrote %s (%d parsed from email, %d hand-entered, %d of those placeholder)"
          % (dest, len(weeks) - len(extra), len(extra),
             sum(1 for w in extra if w["placeholder"])))
