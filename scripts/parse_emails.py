...
def placeholder_weeks(dest):
    """Load hand-entered weeks that sit alongside the parsed ones."""
    path = os.path.join(os.path.dirname(os.path.abspath(dest)), "placeholder-weeks.json")
...
def main(src, dest):
    ...
    extra = [w for w in placeholder_weeks(dest) if w["label"] not in reported]
    weeks.extend(extra)
    ...
    if os.path.isdir(os.path.dirname(js)):
        ...
        print("wrote %s" % js)
    print("wrote %s (%d weeks from email, %d placeholder)"
